...
//	protected int W;  //  the size of lengths
	...
	public Map<String, Double> calculateFrequencies(List<String> inputSequence, List<Integer> lengths, int i, Map<String, Double> Frequencies) {
		int k = inputSequence.size() - lengths.get(i) + 1;
		Map<String, Integer> Ocurrence = new HashMap<String, Integer>();
		for (int j = 0; j < k; ++j) { 
			String shortSequence = ListString.getString( inputSequence, j, j + lengths.get(i) );
			if ( !Ocurrence.containsKey(shortSequence) ) {   //  calculating occurrences
