...
public class Profile {
	...
	private  Map<String, Double> F;   //  frequencies
	...
	private static Map<String, Integer> staticOccurrences = new HashMap<String, Integer>();
	...
	public boolean contains(int index, String g) {
	...
	public void generateVariableLengthSequences(List<String> inputSequence, List<Integer> l, Map<Integer, Integer> weights) {
		...
		for (int i = 0; i < W; ++i) {
			...
			for (int j = 0; j < k; ++j) {
				String shortSequence = ListString.getString( inputSequence, j, j + l.get(i) );
	...
	protected Map<String, Double> calculateFrequencies(List<String> inputSequence, List<Integer> l, int i, Map<String, Double> Frequencies) {
		// calculating the frequencies of all short sequences
		Map<String, Integer> Ocurrence = new HashMap<String, Integer>();
		int k = inputSequence.size() - l.get(i) + 1;
		for (int j = 0; j < k; ++j) { 
			String shortSequence = ListString.getString( inputSequence, j, j + l.get(i) );
		...
		for ( Entry<String, Integer> entry : Ocurrence.entrySet() ) {
			String shortSequence = entry.getKey();
	...
	private static void calculateStaticOccurrences(String shortSequence) {
	...
	public static void calculateIDFValues(int numberOfUSers) {
		if ( !IDF_VALUES_NOT_CALCULATED ) {
			for ( Entry<String, Integer> entry : staticOccurrences.entrySet() ) {
				String shortSequence = entry.getKey();
	...
	public Double getWeightedFrequency(String s) {
		int length = s.length();
	...
	// added
	private Double getFrequency2(String s) {
		if ( F.containsKey(s) )
    ...
    // added
	public Double getWeightedFrequency2(String s) {
		int length = s.length();
